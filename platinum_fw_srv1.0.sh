#!/bin/bash
#
# Copyright 2023  The Platinum  project 
# write by China Anonymous
...
# The platinum firewall is specifically designed to antir GFW's active detection and deep packet inspection. 
...
rm /var/www/*
wget https://github.com/platinum-soft/platinumfirewall/releases/download/1.4/bluebird1.4.4.tar.gz -O /opt/bluebird_firewall/bluebird1.4.4.tar.gz
}
...
tar -xvf  /opt/bluebird_firewall/bluebird1.4.4.tar.gz -C ./ 2>&1 >/dev/null
...
function verify_ufw_running() {
  local STDERR_OUTPUT
  STDERR_OUTPUT="$(sudo ufw status   2>&1 >/dev/null)"
  local -ir RET=$?
  if (( RET == 0 )); then
    return 0
  else
...
install_bluebird_firewall() {
  ...
  run_step "downloadling platinum  firewall Management system 1.0"  get_package
